...
import 'screens/features_button_page.dart'
    as featuresButtonPage; // Alias to resolve ambiguity
...
import 'screens/sera_chatbot.dart' as ashChatbot; // Alias to resolve ambiguity
import 'screens/dialogs.dart'; // Import DialogPage
import 'screens/user_profile_page.dart'; // Import the UserProfilePage
import 'screens/emergency_id_page.dart';
...
Future<void> addContact(String name, String phone) async {
  final response = await http.post(
    Uri.parse(
        'http://192.168.1.4:5000/contacts'), // Ensure the IP address is correct and server is running
...
Future<void> addMedication(String name, String dosage, String time) async {
  final response = await http.post(
    Uri.parse(
        'http://192.168.1.4:5000/medications'), // Ensure the IP address is correct and server is running
    headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
    },
    body: jsonEncode(<String, String>{
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      routes: {
        '/': (context) => WelcomePage(),
        '/feature_buttons': (context) =>
            featuresButtonPage.FeatureButtonsPage(), // Use alias
        '/emergency_id': (context) => EmergencyIdPage(),
